...
void draw_tree(int level, float sx, float sy);
void draw_stem(float sx, float sy);
void draw_leaf(float sx, float sy);
...
void draw_tree(int level, float sx, float sy) {
    ...
    glPushMatrix();
        glTranslatef(0, sy, 0); // Height of the stem is 0.6
        glRotatef(35, 0, 0, 1);

        // Now that we positioned the coordinates as needed, draw the leaf
        if (level > 0) draw_tree(level-1, sx*downscale, sy*downscale);
        else draw_leaf(sx*downscale, sy*downscale);
    glPopMatrix();
